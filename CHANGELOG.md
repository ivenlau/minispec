...
### Added
- Lifecycle commands `minispec upgrade` / `minispec remove` / `minispec uninstall` + a standalone `uninstall.sh` / `uninstall.ps1` at the repo root (mirroring the install bootstrap). `upgrade` refreshes agent-side files (AGENTS.md, CLAUDE.md, and the two SKILL.md mirrors) from the installed CLI share while never touching `minispec/project.md`, `specs/`, `changes/`, or `archive/`. `remove` deletes the whole minispec scaffolding from a project and strips the `# >>> minispec` marker block from `.gitignore`; interactive by default, `--yes` to skip prompt, `--keep-archive` / `--keep-specs` for partial removal. `uninstall` removes the global launcher, share directory, and (Windows) the user PATH entry. All three commands support `--dry-run`. `workflow.md` gains a `## Lifecycle` section with BDD for each command.
